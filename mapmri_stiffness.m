function [RTOP,RTAP,RTPP,NG,NG_par,NG_per,PA_DTI, PA,delta_theta,I_trans,a_coeff,phi,S0,D,V,FA] = ...
    mapmri_stiffness(S,bvals,bvecs,big_delta,little_delta,Nmax,method,n)
...
[alpha] = making_alpha(Dcnls,D0, little_delta, big_delta,S,bvals,bvecs);
[V, D] = eig(alpha);
...
% phi_x_iso = zeros(size(bvecs,2),size(ind,1));
% 
% for j = 1:size(ind,1)
%     phi_x_iso(:,j) = mapmri_phi(ind(j,1),u0,q(1,:));
% end
% 
% phi_iso = phi_x_iso.^3;
