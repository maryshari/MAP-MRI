% function [RTOP,RTAP,RTPP,NG,NG_ver,NG_par,PA_DTI,a_lin,S0,phi,D,V,MD,FA,u0,a_coeff,psi] = ...
function [RTOP,RTAP,RTPP,NG,NG_par,NG_per,PA_DTI, PA,delta_theta, a_coeff,phi,S0]  = ...
    mapmri_fix_D(S,bvals,bvecs,big_delta,little_delta,Nmax,method,D,V)
...
for j = 1:size(ind,1)
    phi_x(:,j) = mapmri_phi(ind(j,1),sqrt(D(1)),q(1,:));
    phi_y(:,j) = mapmri_phi(ind(j,2),sqrt(D(2)),q(2,:));
    phi_z(:,j) = mapmri_phi(ind(j,3),sqrt(D(3)),q(3,:));
end
...
switch method
    case 'MAP'
        ...
        %         A = -[phi ;K1 ; w'*K1];% -eye(size(phi,2))];% -1*ones(1,size(phi,2))];
        %         b = [zeros(size(phi,1),1) ;zeros(size(samples,1),1) ; 0.5];% ones(size(phi,2),1)];% 1];
...
for i = 1:size(ind,1)
    for j = 1:size(ind,1)
        ...
        U(i,j) = u0^3/(u0*u0)*S1(1,i,j)*U1(2,i,j)*U1(3,i,j) +2*u0*u0/u0*T1(1,i,j)*T1(2,i,j)*U1(3,i,j) + ...
            u0^3/(u0*u0)*S1(2,i,j)*U1(3,i,j)*U1(1,i,j) + 2*u0*u0/u0*T1(2,i,j)*T1(3,i,j)*U1(1,i,j) + ...
            u0^3/(u0*u0)*S1(3,i,j)*U1(1,i,j)*U1(2,i,j) + 2*u0*u0/u0*T1(1,i,j)*T1(3,i,j)*U1(2,i,j);
